...
object RailsBridge {
  ...
  def run(gameSave: String, screenshotDir: String): Boolean = {
    ...
    if (!successful) {
      ...
      val delimiter = "=================="
      val body = List(s"Game save: $gameSave", "", "stdout", delimiter) ++ stdoutLines ++ List(
        delimiter,
        "",
        "stderr",
        delimiter
      ) ++ stderrLines ++ List(delimiter)
      EmailContent(
        "jcoveney+poll_rails_game@gmail.com",
        "jcoveney@gmail.com",
        "Errors detected",
        body.mkString("\n"),
        Map()
